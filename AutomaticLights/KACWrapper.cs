...
namespace KACWrapperGEH
{
    ...
    public class KACWrapper
    {
        ...
        internal static void AddPEAlarm(Vessel vessel, string message)
        {
            try
            {
                bool ready = KACWrapper.APIReady;
                if (!ready)
                {
                    ready = KACWrapper.InitKACWrapper();
                }

                if (ready)
                {
                    KACWrapper.KAC.CreateAlarm(KACAPI.AlarmTypeEnum.Periapsis, message, vessel.orbit.GetTimeToPeriapsis());
                }
            }
            catch
            { }
